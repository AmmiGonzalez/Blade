...
<div class="contentList d-flex flex-column">
  ...
  @if (count($usuarios) >= 1)
      ...
      @foreach ($usuarios as $usuario)
        <div class="col-md-2 mt-3 mx-3 card" style="width: 18rem;">
          <a href="/usuarios/{{$usuario->id}}">
            ...
            <div class="card-body">
              <h5 class="card-title"><b>{{$usuario->username}}</b></h5>
              <p class="card-text"> {{$usuario->email}} </p>
            </div>
          </a>
          <div class="content-footer mt-auto d-flex flex-row align-items-center justify-content-start">
            <button onclick="window.location='{{ route('editar.usuario', $usuario) }}'" class="btn btn-warning"><i class="fa-solid fa-pen-to-square"></i></button>
            <button data-nombre="{{$usuario->username}}" data-id="{{$usuario->id}}" class="btn btn-danger delete" data-bs-toggle="modal" data-bs-target="#dashboardModal"><i class="fa-solid fa-trash-can"></i></button>
          </div>
        </div>
      @endforeach
    </div>
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          <div class="modal-header">
            <h1 class="modal-title fs-5" id="dashboardModalLabel">Eliminar la usuario</h1>
    ...
    <div class="container d-flex align-items-center justify-content-center w-100 mt-5">
      <div class="card">
        <b class="text-red my-3 mx-5">No hay usuarios registrados</b>
      </div>
    </div>
  @endif
</div>
<script>
  ...
  $('#btnDelete').on('click', function() {
    $.ajax({
      ...
      url: "/usuarios/eliminar/" + selectedProduct,
      type: 'delete',
