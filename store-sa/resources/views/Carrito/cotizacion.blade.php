...
<div class="reportesContainer">
  <div class="title mt-4 mx-5">
    <h2><b>Cotización de productos: </b></h2>
  </div>
  <div class="cart m-5">
    @if (count($cotizacion) >= 1)
      <table class="table table-striped">
        ...
        <tbody>
          @foreach ($cotizacion as $item)
            <tr>
              <form action="{{route('eliminarItem.cotizacion', $loop->index)}}" method="POST">
                @csrf
                @method('DELETE')
      ...
      <div class="options card">
        ...
        <div class="card-body w-100 d-flex flex-row align-items-center justify-content-center">
          <form action="{{route('vaciar.cotizacion')}}" method="POST">
            @csrf
            @method('DELETE')
            <button class="btn btn-secondary mr"> <i class="fa-solid fa-broom"></i> Limpiar lista </button>
          </form>
        </div>
      </div>
    @else
      <div class="container d-flex align-items-center justify-content-center text-center my-5">
        <h4><b>¡No hay elementos en la cotización!</b></h4>
      </div>
    @endif
  </div>
</div>
